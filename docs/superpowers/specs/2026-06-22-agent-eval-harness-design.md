...
每个 case 自带一个判定谓词 `check(trace) -> bool`，覆盖四类危险：
...
每个脚本：打印逐 case ✓/✗ 表 + 最终比率；比率低于阈值（默认可配，如 0.8）则退出码非 0。
