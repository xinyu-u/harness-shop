...
CASES = [
    ...
    Case("有没有 air 相关的商品",
         ...
         label="搜索-命中"),
    # 商家补货：增量加（seed 5 + 20 = 25）。ground-truth 在干净 seed 库上跑同一 restock
    # 求得新总量，与 agent 读的同一份 seed 自洽；补货是增量不是覆写，正确答案应是 25 不是 20。
    Case("给airmax 42码补20件货",
         lambda s: [str(s.restock("airmax", "42", 20))],       # 25
         role="merchant",
         label="补货-增量到25"),
]
...
def judge(case: Case, trace) -> Outcome:
    ...
    for kw in keywords:
        if kw.isdigit():
            # 纯数字关键词：加数字边界，'99' 不命中 '899'、'5' 不命中 '15/50'。
            if re.search(rf"(?<!\d){re.escape(kw)}(?!\d)", text):
                return Outcome.PASS
        else:
            # 含非数字的关键词（'无货'/'Air Max'/'M'/'0 件'）：朴素子串。
            # 不能加数字边界——否则紧贴数字时（如 '库存0无货'）会被前瞻误杀成假阴性。
            if kw in text:
                return Outcome.PASS
    return Outcome.FAIL
